...
static void i2c_ack(bit ack)	//应答信号，给接收函数用，主机每接收到一个字节要向从机发送一位应答信号，但是最后一个字节不需要应答
{
//进入此函数之前i2c_scl是低电平
	i2c_sda=ack;
	i2c_delay();
...
//进入此函数前i2c_scl和i2c_sda应都是高电平
	...
	i2c_sda=0;
	i2c_delay();i2c_delay();
	i2c_scl=0;	//钳住i2c_scl，这样其它设备就不会占用I2C总线了
	i2c_delay();
...
*/
static bit i2c_sendB(unsigned char dat)	//根据I2C协议，先发高位后发低位
{
...
这样可以自适应速度。		*
void i2c_sendB(unsigned char dat)	//根据I2C协议，先发高位后发低位
{
	void delay5(unsigned char);
	bit ack;	//应答信号
	unsigned char i;
...
static unsigned char i2c_recB(bit ack)	//先传高位，后传低位
{
	unsigned char get=0;
	unsigned char i;
	i2c_sda=1;	//释放i2c_sda以接收数据
	for(i=0;i<=7;i++)
	{
		i2c_delay();
		i2c_scl=1;
